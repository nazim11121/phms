        ...
        return datatables()
            ...
            ->addColumn('action', function ($item) {
                ...
                if (auth()->user()->can('Edit User')) {
                    $buttons .= '<a class="dropdown-item" href="' . route('admin.category.edit', $item->id) . '" title="Edit"><i class="mdi mdi-square-edit-outline"></i>' . __('custom.edit') . '</a>';
                }
                if (auth()->user()->can('Edit User')) {
                    $buttons .= '<form action="' . route('admin.category.destroy', $item->id) . '"  id="delete-form-' . $item->id . '" method="post">
            ...
            //     return '<img class="img-64" src="' . getStorageImage(Staff::FILE_STORE_PATH, $item->avatar) . '" alt="' . $item->name . '" />';
                ...
                $badge = $item->status == Category::Active ? "badge-success" : "badge-danger";
