...
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <h4 class="float-left header-title">{{ __t('group_list') }}</h4>
                {!! $dataTable->table() !!}
            </div>
        </div>
    </div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">{{ __t('edit_group') }}</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <form class="form-validate" action="{{ route('admin.group.data.update')}}" method="POST" enctype="multipart/form-data">
        @csrf   
        <div class="modal-body">
            <div class="row">
                
                <div class="form-group col-sm-6 mt-2">
                    <label for="name">Medicine Group Name <span class="requiredStar"> *</span></label>
...
<script>
  ...
  function openEditModal(button) {
    ...
    $.ajax({
        url: 'group/' + valueToDisplay2+'/edit/',
        method: 'GET',
        // dataType: 'json',
