...
<div class="row">
    <div class="col-12">
        <div class="card" style="background: black;color: red;">
            <div class="card-body">
                <h4 class="header-title">{{ __('Online Delivery Order') }}</h4>
                ...
                <form class="form-validate" action="{{ route('admin.order.store') }}" method="POST"
                    ...
                    <div class="container mt-5">
                        <div class="row">
                        <!-- Category List -->
                            <input type="hidden" name="order_type" value="Online Delivery">
...
<script>
        $(document).ready(function () {
            ...
            function calculateGrandTotal() {
                ...
                if(deliveryCharge>0  && discount<1){
                    var grandTotalDis = parseFloat(grandTotal)+parseFloat(deliveryCharge);
                    $('#grandTotal').text('৳' + grandTotalDis.toFixed(2));
                    $('#grandTotals').val(grandTotalDis);
                }else if(deliveryCharge>0 && discount>0){console.log(discount);
                    var grandTotalDis = (parseFloat(grandTotal)+parseFloat(deliveryCharge))-(discount);
                    $('#grandTotal').text('৳' + grandTotalDis.toFixed(2));
                    $('#grandTotals').val(grandTotalDis);
                }else if(discount>0 && deliveryCharge<1){
